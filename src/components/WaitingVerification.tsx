  ...
  return (
    <div className="space-y-6 py-1 text-slate-100" id="waiting-verification-panel">
      ...
      <div className="text-center space-y-4 pt-1">
        {/* Simplified corporate spinning block loader */}
        ...
        <div className="space-y-2 font-sans">
          ...
          <p className="text-slate-350 text-xs md:text-sm max-w-sm mx-auto leading-relaxed">
            ...
            <span className="block text-rose-400 font-semibold mt-1 break-all font-sans">{email || 'sso-github@vyzorix.com'}</span>
          </p>
        </div>
      ...
      {/* Elegant Corporate Status Container */}
      <div className="border border-white/10 rounded-xl p-5 bg-slate-950/40 space-y-3.5 text-xs text-left">
        <div className="flex items-center gap-2 border-b border-white/5 pb-2">
          <ShieldCheck className="w-4 h-4 text-rose-500 shrink-0" />
          <span className="font-semibold text-slate-200 uppercase tracking-wider text-[10px]">Verification Service</span>
        </div>
        ...
        {/* Setting up profile... inline indicator replacing dynamic log area */}
        <div className="p-3 bg-rose-950/10 border border-rose-500/10 rounded-lg text-xs flex items-center gap-2.5">
          ...
          <span className="font-semibold text-slate-300">{statusText || 'Setting up profile...'}</span>
        </div>
      </div>
