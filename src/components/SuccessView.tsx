  ...
  return (
    <div className="space-y-8 text-slate-100" id="success-view">
      <div className="text-center space-y-3">
        {/* Verification Check Badge in Rose with no green alerts */}
      ...
      <div className="border border-white/10 rounded-2xl p-6 bg-slate-950/40 space-y-5 shadow-2xl relative before:absolute before:inset-0 before:bg-gradient-to-b before:from-white/5 before:to-transparent before:rounded-2xl before:pointer-events-none">
        ...
        <div className="space-y-4 text-xs font-sans">
          ...
          {/* Vyzorix member (was username) */}
          <div className="flex justify-between items-center py-1">
            <span className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Vyzorix member</span>
            <span className="text-white font-semibold text-sm">@{successReport.username || 'google_member'}</span>
          </div>

          {/* Your email */}
          <div className="flex justify-between items-center py-1 border-t border-white/5 pt-3">
            <span className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Your email</span>
            <span className="text-white font-medium select-all break-all">{successReport.email || 'sso-google@vyzorix.com'}</span>
          </div>

          {/* Member ID */}
          <div className="flex justify-between items-center py-1 border-t border-white/5 pt-3">
            <span className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">ID</span>
            <span className="text-slate-200 font-mono font-bold">VXZ-64981</span>
          </div>

          {/* Operator */}
          <div className="flex justify-between items-center py-1 border-t border-white/5 pt-3">
            <span className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Operator</span>
            <span className="text-rose-400 font-semibold">Operator</span>
          </div>

          {/* Region */}
          <div className="flex justify-between items-center py-1 border-t border-white/5 pt-3">
            <span className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Region</span>
            <span className="text-slate-300 font-medium">{successReport.region || 'Paris, Île-de-France, France'}</span>
          </div>

          {/* Session Date */}
          <div className="flex justify-between items-center py-1 border-t border-white/5 pt-3">
            <span className="text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Session Date</span>
