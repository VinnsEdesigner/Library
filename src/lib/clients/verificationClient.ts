 ...
 */
export async function pollVerificationStatus(token: string): Promise<{ status: 'waiting' | 'success'; report?: SuccessReport }> {
  if (IS_SIMULATED) {
 ...
 */
export async function triggerTokenResend(email: string): Promise<{ success: boolean; message: string }> {
  if (IS_SIMULATED) {
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({
