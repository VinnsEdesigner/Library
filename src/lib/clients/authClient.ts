 ...
 */
export async function registerOperator(payload: SignUpPayload): Promise<{ success: boolean; message: string }> {
  if (IS_SIMULATED) {
...
export async function loginOperator(usernameOrEmail: string, passwordSecret: string): Promise<SuccessReport> {
  if (IS_SIMULATED) {
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({
...
export async function requestPasswordReset(emailAddress: string): Promise<{ success: boolean; message: string }> {
  if (IS_SIMULATED) {
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({
