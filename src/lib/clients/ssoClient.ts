  ...
  if (IS_SIMULATED) {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const fullName = provider === 'Google' ? 'Google Authenticated User' : 'GitHub Operator Tech';
 ...
 */
export async function handleSSOCallback(provider: 'Google' | 'GitHub', code: string, state: string): Promise<SuccessReport> {
  if (IS_SIMULATED) {
    return initiateSSO(provider);
  }

  const response = await fetch(`${API_BASE}/sso/${provider.toLowerCase()}/callback?code=${code}&state=${state}`);
  if (!response.ok) {
