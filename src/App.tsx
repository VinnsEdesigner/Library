...
export default function App() {
  ...
  const [profileData, setProfileData] = useState<{
    ...
    const hydrated = getHydratedState<{ fullName: string; email: string; username: string } | null>('profileData', null);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const ssoProvider = params.get('provider') || (window.location.search.includes('github') ? 'GitHub' : 'Google');
  ...
  return (
    ...
    >
      ...
      {toast && (
        <div 
          className="fixed top-8 left-1/2 -translate-x-1/2 z-50 px-6 py-4 rounded-xl border-t-2 bg-slate-950/95 border-rose-600/40 text-slate-100 shadow-2xl flex items-center gap-4 text-xs max-w-sm w-[90%] transition-all animate-fade-in"
        >
      ...
      <header className="w-full top-0 sticky z-40 bg-slate-950/40 backdrop-blur-md border-b border-white/5">
        <div className="flex justify-between items-center px-8 py-5 max-w-7xl mx-auto w-full">
          ...
          >
            ...
            <span className="font-extrabold text-xl tracking-wider uppercase text-white font-sans">Vyzorix</span>
          </div>
          
          <div className="flex items-center gap-5">
            {view === 'signup' && (
              <>
                <span className="text-slate-400 text-xs hidden sm:inline font-normal">Already have an account?</span>
                <button 
            ...
            {view === 'login' && (
              <>
                <span className="text-slate-400 text-xs hidden sm:inline font-normal">New to Vyzorix?</span>
                <button 
        ...
        <div className="max-w-7xl mx-auto px-8 flex flex-col md:flex-row justify-between items-center gap-4 text-slate-400 text-xs font-normal">
          ...
          <div className="flex gap-4">
            <a 
              ...
              href="#privacy" 
              onClick={(e) => { e.preventDefault(); triggerToast("Zero tracking logs cached.", "success"); }}
            >
              Privacy Policy
            </a>
            <a 
